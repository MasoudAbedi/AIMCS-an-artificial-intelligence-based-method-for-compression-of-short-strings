...
namespace AIMCSA
{
    public class AIMCSAv1
    {
        char[] char_ = new char[256];
        int[] index_ = new int[256];
        int[] useOfChar = new int[256];
        int index = 1;
        ...
        {
            string Rez = "0";
            Rez += convertorInt2Bin(0, 4, 0);
            for (int i = 1; i < index; i++)
                Rez += convertorInt2Bin(char_[i], 8, 0);
            Rez += "000";
        ...
        string CompressStr(string InStr, int mode, bool AddNall)
        {
            ...
            if (CompressIsMood)
            {
                ...
                Rez = "1";
                Rez += convertorInt2Bin(MaxLength, 4, 0);
            }
            else
            {
        ...
        public string Decompress(byte[] InByte)
        {
            ...
            {
                string T_ = "";
                for (int i = 5; i < RezStringByte.Length - 3; i++)
                    T_ += RezStringByte[i];
                byte[] byteTable = convertorByteString2byte(T_);
                index = byteTable.Length + 1;
                for (int i = 1; i < index; i++)
                    char_[i] = Convert.ToChar(byteTable[i - 1]);// newString[i];
                return null;
            }
            if (RezStringByte[0] == 48 && readLength == 0)
            {
                int indexReSort = 1;
                string temp_char = "";
                for (int i = 5; i < RezStringByte.Length - 3; i += 8)
                {
                    temp_char = "";
                    for (int k = i; k < 8 + i; k++)
            ...
            bool Fl = false;
            int Cont_Heder = 0;
            for (int i = 5; i < RezStringByte.Length; )
            {
                temp = "";
                Fl = false;
                Cont_Heder = i;
                ...
                if (convertorBin2Int(temp) == 0)
                {
                    ...
                    if (RezStringByte[i] == '1')
                        ...
                        CompressIsMood = false;
                    for (int h = i + 1; h < RezStringByte.Length && h < i + 5; h++)
